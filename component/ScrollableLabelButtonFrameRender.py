...
from component.RenderItem import RenderItem
...
class ScrollableLabelButtonFrameRender(ctk.CTkScrollableFrame):
    def __init__(self, master, edit_command=None, remove_command=None, upload_command=None, **kwargs):
        ...
        self.header_label.grid(row=0, column=0, columnspan=9, pady=(0, 10), sticky="ew")
    ...
    def add_item(self, item_id, item_name, progress=0, image=None):
        item = RenderItem(self, item_id, item_name, progress, image)
        ...
        item.upload_link_button.configure(command=lambda: self.copy_to_clipboard(f"{UPLOAD_LINK}{item_id}.zip"))

        self.items.append(item)

        item.row_index = len(self.items)

        item.frame.grid(row=item.row_index, column=0, columnspan=2, pady=(0, 10), sticky="ew")
